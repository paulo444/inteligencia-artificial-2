...
namespace Practica_1___IA_2
{
    ...
    public partial class MainForm : Form
    {
        ...
        void ButtonCreateArquitectureClick(object sender, EventArgs e)
        {
            ...
            network = new ActivationNetwork(function, 2, layers.ToArray());

            new NguyenWidrow(network).Randomize();

            lm = new LevenbergMarquardtLearning(network);
        }
        ...
        double[][] toDoubleMatrix(int[][] d)
        {
            double [][] a = new double[d.GetUpperBound(0)+1][];

            for(int i=0; i<d.GetUpperBound(0)+1; i++)
            {
                a[i] = new double[d.GetUpperBound(1) + 1];
                for(int j=0; j<d.GetUpperBound(1)+1; j++)
                {
                    a[i][j] = d[i][j];
                }
            }
        ...
        int[] createOutput()
        {
            int[] a = new int[points.Count];

            for (int i = 0; i < points.Count; i++)
            {
                a[i] = Convert.ToInt32(points[i].V);
            }
                ...
                for (int j = 0; j < WIDTH; j++)
                {
                    if (bitmap.GetPixel(j, i).R == 255 && bitmap.GetPixel(j, i).G == 255 && bitmap.GetPixel(j, i).B == 255)
                    {
                        ...
                        */
                        ...
                        for (int k = 0; k < output.GetUpperBound(0) + 1; k++)
                        {
                            if (color < output[k])
                            {
                                colorIndex = k;
                        ...
                        if (classNumber <= 7)
                        {
                            ...
                            {
                                bitmap.SetPixel(j, i, Color.FromArgb(0, 200, 0));
                            }
                            ...
                            {
                                bitmap.SetPixel(j, i, Color.FromArgb(200, 200, 0));
                            }
                            else if (colorIndex - 1 == 4)
                            {
                                bitmap.SetPixel(j, i, Color.FromArgb(200, 0, 200));
                            }
                            else if (colorIndex - 1 == 5)
                            {
                                bitmap.SetPixel(j, i, Color.FromArgb(0, 200, 200));
                            }
                            else if (colorIndex - 1 == 6)
                            {
                                bitmap.SetPixel(j, i, Color.FromArgb(200, 200, 200));
                            }
                        }
                        else
                        {
        ...
        void setResults()
        {
            ...
            for (int i = 0; i < points.Count; i++)
            {
                ...
                results[(int)(rp.V), colorIndex] = results[(int)(rp.V), colorIndex] + 1;
            }
